<section>
    ...
    <form method="post" action="{{ route('symbol.store') }}" class="mt-6 space-y-6">
        @csrf
        @method('post')

        <div>
            <x-input-label for="symbol" :value="__('Symbol')" />
            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" required autocomplete="symbol name" />
